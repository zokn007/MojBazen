const CACHE_VERSION='mojbazen-v4-11';
const ASSETS=['./','./index.html','./styles.css','./app.js','./manifest.json','./cloud-sync.js','./update-manager.js','./version.json','./icon-192.png','./icon-512.png','./apple-touch-icon.png','./favicon-32.png','./mojbazen-icon.png'];
...
self.addEventListener('fetch',e=>{
  ...
  if(u.pathname.endsWith('/version.json')||e.request.mode==='navigate'){
    ...
    e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request).then(resp=>{const copy=resp.clone();caches.open(CACHE_VERSION).then(c=>c.put(e.request,copy));return resp}).catch(()=>caches.match('./index.html'))));
  }
});
